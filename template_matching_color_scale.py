...
## choose image here (might aswell make a function out of the whole file)
...
#template = cv2.imread("D://Jannes//Dokumente//Uni//introCV//f_templates//diamond.png", cv2.COLOR_BGR2RGB)
...
# bestmatch = [0,cv2.resize(template, (25, 25))] # best match : matchvalue, croppedpart ;; not needed if using threshhold
# now you can access it outside the "for loop..."
for template in templates:
    #use extra for loop if we want more resizes
    template = cv2.resize(template, (25, 25))
    ...
    #continue if match is below thresh (depends on methode used)
    if max_val < thresh:
        continue
    ...
    #add cropped image into bestmatch, it it is the best match.
    #if bestmatch[0]
